...
public class FilmCatalog {
	...
	public ArrayList<Film> findFilmByStrategy(SelectorStrategy strategy) {
		// System.out.println("HereSS2");
		ArrayList<Film> foundFilms = new ArrayList<Film>();
		Iterator<Film> it = films.iterator();
		while (it.hasNext()) {
			Film aFilm = it.next();
			// System.out.println(aFilm.getTitle());
			if (strategy.meetsCriteria(aFilm))
				foundFilms.add(aFilm);
		}
		if (foundFilms.size() > 0)
			return foundFilms;
		System.out.println("FilmCatalog.findFilmsByStrategy: using " + strategy + ". No films found.");
		return null;
	}
}
