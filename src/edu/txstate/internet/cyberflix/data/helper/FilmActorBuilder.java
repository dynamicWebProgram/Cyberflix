...
public class FilmActorBuilder {
	...
	public void build(FilmCatalog filmInventory, ActorInventory actorInventory,
			List<SimpleEntry<Integer, Integer>> pairs) {
		...
		while (iterator.hasNext()) {
			...
			if ((anActor != null) && (aFilm != null)) {
				...
				// String actorName = anActor.getFirstName() + " " +
				// anActor.getLastName();
				// String filmTitle = aFilm.getTitle();
			}
			else {
				if (anActor == null)
					System.err.println(CLASS_NAME + " did not find actor with id " + actorID);
				if (aFilm == null)
					System.err.println(CLASS_NAME + " did not find film with id " + filmID);
			}
